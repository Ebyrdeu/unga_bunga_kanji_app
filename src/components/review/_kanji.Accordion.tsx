...
export const KanjiAccordion = ({kanjiData}: { kanjiData: UserKanji }) => {
  const {disabled} = useKanjiGameStore();
  return (

      <Accordion sx={{visibility: !disabled ? 'hidden' : 'visible'}} defaultValue={'readings'} mt={'md'} radius={0}
                 variant={'contained'}>
        <Accordion.Item value="meanings">
          ...
          <Accordion.Panel>
            ...
            <Text sx={{
              display: kanjiData[0]?.kanji.meanings.slice(1).length === 0
                  ? 'none'
                  : 'block',
            }}>Alternative: {kanjiData[0]?.kanji.meanings.slice(1).join(', ')}</Text>
          </Accordion.Panel>
        </Accordion.Item>
